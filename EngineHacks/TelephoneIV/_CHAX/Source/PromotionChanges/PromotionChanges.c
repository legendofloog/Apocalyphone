...
const struct PromotionBonuses* GetUnitPromotionBonuses(struct Unit* unit)
{
    int i = 0;
    while(1){
        if (UnitPromotionBonusTable[i].charID == 0){
            return 0;
        }
        if (UnitPromotionBonusTable[i].charID == unit->pCharacterData->number) 
            { return &UnitPromotionBonusTable[i]; }
        i++;
    }
...
void ApplyUnitPromotionSpecific(struct Unit* unit, const struct ClassData* promotedClass, const struct PromotionBonuses* bonuses)
{
    ...
    if (unit->ranks[ITYPE_SWORD] > WPN_EXP_S) 
        { unit->ranks[ITYPE_SWORD] = WPN_EXP_S; }

    unit->ranks[ITYPE_LANCE] += bonuses->lncWexp;
    if (unit->ranks[ITYPE_LANCE] > WPN_EXP_S) 
        { unit->ranks[ITYPE_LANCE] = WPN_EXP_S; }
    ...
    if (unit->ranks[ITYPE_ANIMA] > WPN_EXP_S) 
        { unit->ranks[ITYPE_ANIMA] = WPN_EXP_S; }

    unit->ranks[ITYPE_LIGHT] += bonuses->lgtWexp;
    if (unit->ranks[ITYPE_LIGHT] > WPN_EXP_S) 
        { unit->ranks[ITYPE_LIGHT] = WPN_EXP_S; }

    unit->ranks[ITYPE_DARK] += bonuses->drkWexp;
    if (unit->ranks[ITYPE_DARK] > WPN_EXP_S) 
        { unit->ranks[ITYPE_DARK] = WPN_EXP_S; }

    unit->ranks[ITYPE_STAFF] += bonuses->stfWexp;
    if (unit->ranks[ITYPE_STAFF] > WPN_EXP_S) 
